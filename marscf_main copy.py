...
class Quantizer(nn.Module):
	def __init__(self, embed_space):
		super().__init__()
		self.num_embed = embed_space.shape[0]
		self.embed_dim = embed_space.shape[1]
	...
	def forward(self, input_data):
		...
		dist = (
            ...
            2 * data @ self.embed_space.transpose(0, 1) + \
            self.embed_space.pow(2).sum(1).reshape(1, num_embed).repeat(data.shape[0], 1)
        )
		embed_ind = torch.argmin(dist, dim = 1)
		quantize = F.embedding(embed_ind, self.embed_space)
...
if __name__ == "__main__":
	...
	embed_space = torch.randn(num_embed, 48, dtype = torch.float32, requires_grad = True)
	if torch.cuda.is_available():
		embed_space = embed_space.cuda()
	quantizer = Quantizer(embed_space)
	embed_buffer = embed_space.clone().detach().cpu().numpy()
	...
	if not from_checkpoint:
		...
		optimizerG = optim.Adamax(list(mar_scf.parameters()) + [embed_space],lr=5*1e-6)#.cuda(2)
		...
		for epoch in range(epochs):
			...
			for data in train_bar:
				...
				if torch.cuda.is_available():
					print(((torch.tensor(embed_buffer) == embed_space.cpu()) == False).sum())
				else:
					print(((torch.tensor(embed_buffer) == embed_space) == False).sum())
